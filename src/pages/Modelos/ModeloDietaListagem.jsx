...
// ─── Modal: Editar metadados do modelo ────────────────────────────────────────
...
const ModalEditarModelo = ({ modelo, onClose, onSalvo }) => {
  ...
  const handleSalvar = async () => {
    ...
    setSalvando(true)
    try {
      const atualizado = await salvarModeloDieta(modelo.name, {
        titulo: titulo.trim(),
    ...
    >
      <div className="p-5 space-y-4">
        <FormGroup label="Título" required>
          <Input value={titulo} onChange={setTitulo} placeholder="Ex: Dieta Hipertrofia 2500 kcal" />
        </FormGroup>
        <FormGroup label="Categoria">
          <Select
            value={categoria}
            onChange={setCategoria}
            options={CATEGORIAS_DIETA}
...
// ─── Modal: Aplicar modelo (criar nova Dieta a partir do template) ────────────

const ModalAplicarModelo = ({ modelo, onClose, onAplicado }) => {
  const [aluno, setAluno] = useState(null)
  const [date, setDate] = useState(todayYMD())
  const [finalDate, setFinalDate] = useState(addDaysYMD(30))
  ...
  const handleAplicar = async () => {
    ...
    setAplicando(true)
    try {
      const modeloCompleto = await buscarModeloDieta(modelo.name)
      const snapshot = JSON.parse(modeloCompleto.snapshot_json || '{}')

      // Busca antropometria do aluno destino (mesma lógica de duplicarDieta)
      ...
      const nova = await criarDieta(payload)
...
export default function ModeloDietaListagem() {
  ...
  const handleExcluir = async (modelo) => {
    ...
    try {
      await excluirModeloDieta(modelo.name)
      await carregar()
    } catch (e) {
  ...
  const colunas = [
    ...
    },
    {
      ...
      render: (m) => <span className="text-gray-300 text-xs">{m.strategy_ref || '—'}</span>,
    },
    {
      label: 'Calorias',
      ...
      render: (m) => (
        <div className="flex items-center gap-1.5 text-orange-400">
          <Flame size={12} />
          <span className="text-sm font-medium">{m.total_calories_ref ?? '—'}</span>
    ...
    <div className="p-4 md:p-8 text-white">
      ...
      <div className="max-w-screen-xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-[18px] md:text-xl font-bold text-white tracking-tight">Modelos de Dieta</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-2 mb-4">
          ...
          <div className="w-full sm:w-56">
            <Select
              value={categoria}
              onChange={setCategoria}
              options={['', ...CATEGORIAS_DIETA].map(c => ({ value: c, label: c || 'Todas as categorias' }))}
          ...
          <EmptyState
            ...
            description={
              query || categoria
                ...
                : 'Abra uma dieta e clique em "Salvar como modelo" para começar.'
            }
          />
        ) : (
          <DataTable
