...
// ─── Tabela ──────────────────────────────────────────────────────────────────
export function HistoricoTabela({ schedule, ehTreino }) {
  const dados = schedule.dates.filter(d => !d.is_start)
  if (dados.length === 0) return <p className="text-gray-500 text-xs italic text-center py-4">Sem histórico ainda.</p>
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-xs text-left">
        <thead>
          <tr className="border-b border-[#323238]">
            <th className="px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-500">Data Prevista</th>
...
// ─── Timeline ────────────────────────────────────────────────────────────────
export function HistoricoTimeline({ schedule, ehTreino }) {
  const dados = schedule.dates.filter(d => !d.is_start)
  if (dados.length === 0) return <p className="text-gray-500 text-xs italic text-center py-4">Sem histórico ainda.</p>
  return (
    <div className="relative pl-6 space-y-4">
      ...
      {dados.map(d => {
        const isTr = ehTreino(d.formulario)
        const isDone = d.status === 'Respondido' || d.status === 'Concluido'
        const dotColor = isDone ? 'bg-green-500' : isTr ? 'bg-purple-500' : 'bg-blue-500'
        return (
          <div key={d.date} className="relative">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-1">
              <span className="text-white text-xs font-bold">{fmtDateBR(d.date)}</span>
              <span className={`text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded border ${
                isTr
                  ? 'text-purple-400 bg-purple-500/10 border-purple-500/20'
                  : 'text-orange-400 bg-orange-500/10 border-orange-500/20'
              ...
              {d.respondido_em && (
                <span className="text-[10px] text-gray-500">resp. {fmtDateTimeBR(d.respondido_em)}</span>
              )}
            </div>
...
export function HistoricoRapido({ alunoId, formulariosPorId, showToast, ehTreino }) {
