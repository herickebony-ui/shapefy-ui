  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        ref_weight: Number(form.ref_weight) || 100,
        unit: form.unit || 'g',
        calories: Number(form.calories) || 0,
        protein: Number(form.protein) || 0,
        carbohydrate: Number(form.carbohydrate) || 0,
        lipid: Number(form.lipid) || 0,
        fiber: Number(form.fiber) || 0,
        enabled: form.enabled,
        public: form.public,
      }
      if (isEdit) {
        await salvarAlimento(alimento.name, payload)
      } else {
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <FormGroup label="Grupo Alimentar" required>
            <Select
              ...
              onChange={v => set('food_group', v)}
              options={grupoOpts}
              placeholder="Selecione..."
            />
          </FormGroup>
          <div className="grid grid-cols-2 gap-2">
            <FormGroup label="Peso ref." hint="gramas">
              <Input
                type="number"
                value={String(form.ref_weight)}
                onChange={v => set('ref_weight', v)}
                placeholder="100"
              />
            </FormGroup>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
            ...
            <FormGroup label="Proteína (g)">
              <Input type="number" value={String(form.protein)} onChange={v => set('protein', v)} placeholder="0" />
            </FormGroup>
            <FormGroup label="Carboidrato (g)">
              <Input type="number" value={String(form.carbohydrate)} onChange={v => set('carbohydrate', v)} placeholder="0" />
            </FormGroup>
            <FormGroup label="Lipídeo (g)">
              <Input type="number" value={String(form.lipid)} onChange={v => set('lipid', v)} placeholder="0" />
            </FormGroup>
            <FormGroup label="Fibra (g)">
              <Input type="number" value={String(form.fiber)} onChange={v => set('fiber', v)} placeholder="0" />
            </FormGroup>
          </div>
        </div>
        ...
        <div className="flex gap-6 pt-1">
          ...
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <button
              type="button"
              onClick={() => set('public', form.public ? 0 : 1)}
...
export default function AlimentosListagem() {
  ...
  useEffect(() => {
    listarGruposAlimentares().then(setGrupos).catch(console.error)
  }, [])
      ...
      <ListPage
        ...
        filters={[
          ...
          },
          {
            type: 'select',
            value: grupo,
            onChange: v => { setGrupo(v); setPage(1) },
            options: grupoOpts,
            placeholder: 'Grupo',
          },
          {
