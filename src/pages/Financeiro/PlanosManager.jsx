...
/**
 * Conteúdo gerencial de planos (lista + form de criação/edição) com footer
 * próprio. Usado tanto dentro de Modal (PlanosModal) quanto em página
 * dedicada (PlanosListagem).
 *
 * Props:
 *  - planos: lista resumida (vinda de listarPlanos)
 *  - onMutate: callback após criar/editar/excluir
 *  - onCloseRequest: opcional. Se fornecido, mostra "Fechar" no view list.
 *  - active: false desliga o state interno (otimização para modal fechado)
 */
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="border-t border-[#323238] px-4 py-3 flex items-center gap-2 justify-end bg-[#1a1a1a]/40">
        {view === 'form' ? (
          <>
            <Button variant="ghost" icon={ChevronLeft} onClick={voltar} disabled={salvando}>
              Voltar
            </Button>
          ...
          <>
            {onCloseRequest && (
              <Button variant="ghost" onClick={onCloseRequest}>Fechar</Button>
            )}
